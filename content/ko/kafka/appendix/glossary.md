...
→ [심화 개념](/docs/concepts/advanced-concepts/#acks-acknowledgment)

### Auto Offset Reset
Consumer Group이 처음 시작하거나 Offset 정보가 없을 때 읽기 시작할 위치. `earliest` 또는 `latest`.
→ [Consumer Group & Offset](/docs/concepts/consumer-group-offset/#autooffsetreset-설정)

## B

### Broker
Kafka 서버. 메시지를 저장하고 Consumer에게 전달하는 역할.
→ [핵심 구성요소](/docs/concepts/core-components/#3-broker-브로커)
...
→ [Consumer Group & Offset](/docs/concepts/consumer-group-offset/#offset-커밋)

### Consumer
Kafka에서 메시지를 읽어가는 클라이언트 애플리케이션.
→ [핵심 구성요소](/docs/concepts/core-components/#2-consumer-소비자)

### Consumer Group
같은 목적을 가진 Consumer들의 논리적 그룹. 그룹 내에서 Partition이 분배됨.
→ [Consumer Group & Offset](/docs/concepts/consumer-group-offset/)

## D

### Dead Letter Topic (DLT)
처리에 실패한 메시지를 저장하는 별도의 Topic.
→ [기본 예제](/docs/examples/basic/#dead-letter-topic-dlt)
...
→ [Replication](/docs/concepts/replication/#leader와-follower)
...
→ [Replication](/docs/concepts/replication/#isr-in-sync-replicas)

## K

### KafkaListener
Spring Kafka의 Consumer 어노테이션. 특정 Topic의 메시지를 수신.
→ [기본 예제](/docs/examples/basic/#기본-kafkalistener)

### KafkaTemplate
Spring Kafka의 Producer 클래스. 메시지 전송에 사용.
→ [기본 예제](/docs/examples/basic/#kafkatemplate-주입)

### KRaft
Zookeeper 없이 Kafka 자체적으로 메타데이터를 관리하는 모드. Kafka 3.3+에서 권장.
→ [Replication](/docs/concepts/replication/#zookeeper-vs-kraft)
...
→ [Replication](/docs/concepts/replication/#leader-election)

### Log Compaction
같은 Key의 메시지 중 최신 값만 유지하는 보관 정책.
→ [심화 개념](/docs/concepts/advanced-concepts/#log-compaction)

## M

### Message Key
메시지를 특정 Partition으로 라우팅하는 데 사용. 같은 Key는 같은 Partition으로.
→ [심화 개념](/docs/concepts/advanced-concepts/#message-key)

## O

### Offset
Partition 내 메시지의 순차적 위치 번호. 0부터 시작하여 증가.
→ [Consumer Group & Offset](/docs/concepts/consumer-group-offset/#offset이란)

## P

### Partition
Topic을 분할한 단위. 병렬 처리의 기본 단위.
→ [핵심 구성요소](/docs/concepts/core-components/#5-partition-파티션)

### Producer
Kafka에 메시지를 발행하는 클라이언트 애플리케이션.
→ [핵심 구성요소](/docs/concepts/core-components/#1-producer-생산자)

### Pull 방식
Consumer가 Broker에서 메시지를 가져오는 방식. Kafka는 Pull 방식 사용.
→ [메시지 흐름](/docs/concepts/message-flow/#pull-vs-push)

## R

### Rebalancing
Consumer Group 내에서 Partition을 재분배하는 과정. Consumer 추가/제거 시 발생.
→ [Consumer Group & Offset](/docs/concepts/consumer-group-offset/#리밸런싱-rebalancing)

### Replication Factor
각 Partition의 복제본 수. 프로덕션에서는 3 권장.
→ [Replication](/docs/concepts/replication/#replication-factor)

### Retention
메시지 보관 정책. 시간 기반, 용량 기반, Compaction 방식이 있음.
→ [심화 개념](/docs/concepts/advanced-concepts/#retention-보관-정책)
...
→ [핵심 구성요소](/docs/concepts/core-components/#4-topic-토픽)
